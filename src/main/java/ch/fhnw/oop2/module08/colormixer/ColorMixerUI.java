...
public class ColorMixerUI extends GridPane {
	...
	private ColorMixerPM mixerPM;


	public ColorMixerUI(ColorMixerPM mixerPM) {
		this.mixerPM = mixerPM;
        initializeSelf();
        initializeControls();
		layoutControls();
		setupEventHandlers();
	...
	private void setupBindings() {
		redSlider.valueProperty().bindBidirectional(mixerPM.redProperty());
		redLabel.textProperty().bind(mixerPM.redProperty().asString());
		greenSlider.valueProperty().bindBidirectional(mixerPM.redProperty());
		greenLabel.textProperty().bind(mixerPM.redProperty().asString());
		blueSlider.valueProperty().bindBidirectional(mixerPM.redProperty());
		blueLabel.textProperty().bind(mixerPM.redProperty().asString());

		display.fillProperty().bind(mixerPM.mixedColorProperty());
